setwd('/Users/michaelharvey/Documents/research/Tyranni/v2/tree_analyses/summary_stats/nh/tree_plot')
getwd()

library(ape)
library(phytools)


# AOSHM:

# Tree
tree <- read.tree('~/Documents/research/Tyranni/v2/tree_building/examl/final_timetrees/T400F_AOS_HowardMoore.tre')
tree <- force.ultrametric(tree)
tree <- ladderize(tree)

# Get es values
nh.table <- read.table("../text_files/nh_mean_median_AOSHM.txt")
nh <- nh.table$ht.median
names(nh) <- rownames(nh.table)

# Plot (based on code from Rafael Maia)
bardata <- nh[tree$tip.label]

# plot tree
pdf("AOSHM_median.pdf", width=8, height=8)
par(mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(tree, type='fan', show.tip.label=F, edge.width=0.5, x.lim=c(-70,70), y.lim=c(-70,70), edge.color="gray")
# set angles, convert to radians
angle <- seq(0,360, length.out=Ntip(tree)+1)  # to rotate so it starts counting on right
angle <- angle[-length(angle)] # remove last one so it doesn't overlap with first one
angle <- angle*pi/180 # convert to radians
start <- 45 # set starting point for bars where tips end
scaledat <- .5 # scale to apply so its not disproportional to plotting area
radius <- (bardata*scaledat) + start # "radius" determines size of bars
segments(cos(angle)*start, sin(angle)*start, cos(angle)*radius, sin(angle)*radius, lwd=0.5, col="black", lend=1)
dev.off()

# AOSC:

# Tree
tree <- read.tree('~/Documents/research/Tyranni/v2/tree_building/examl/final_timetrees/T400F_AOS_Clements.tre')
tree <- force.ultrametric(tree)
tree <- ladderize(tree)

# Get es values
nh.table <- read.table("../text_files/nh_mean_median_AOSC.txt")
nh <- nh.table$ht.median
names(nh) <- rownames(nh.table)

# Plot (based on code from Rafael Maia)
bardata <- nh[tree$tip.label]

# plot tree
pdf("AOSC_median.pdf", width=8, height=8)
par(mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(tree, type='fan', show.tip.label=F, edge.width=0.5, x.lim=c(-70,70), y.lim=c(-70,70), edge.color="gray")
# set angles, convert to radians
angle <- seq(0,360, length.out=Ntip(tree)+1)  # to rotate so it starts counting on right
angle <- angle[-length(angle)] # remove last one so it doesn't overlap with first one
angle <- angle*pi/180 # convert to radians
start <- 45 # set starting point for bars where tips end
scaledat <- .5 # scale to apply so its not disproportional to plotting area
radius <- (bardata*scaledat) + start # "radius" determines size of bars
segments(cos(angle)*start, sin(angle)*start, cos(angle)*radius, sin(angle)*radius, lwd=0.5, col="black", lend=1)
dev.off()

# IUCN:

# Tree
tree <- read.tree('~/Documents/research/Tyranni/v2/tree_building/examl/final_timetrees/T400F_IUCN.tre')
tree <- force.ultrametric(tree)
tree <- ladderize(tree)

# Get es values
nh.table <- read.table("../text_files/nh_mean_median_IUCN.txt")
nh <- nh.table$ht.median
names(nh) <- rownames(nh.table)

# Plot (based on code from Rafael Maia)
bardata <- nh[tree$tip.label]

# plot tree
pdf("IUCN_median.pdf", width=8, height=8)
par(mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(tree, type='fan', show.tip.label=F, edge.width=0.5, x.lim=c(-70,70), y.lim=c(-70,70), edge.color="gray")
# set angles, convert to radians
angle <- seq(0,360, length.out=Ntip(tree)+1)  # to rotate so it starts counting on right
angle <- angle[-length(angle)] # remove last one so it doesn't overlap with first one
angle <- angle*pi/180 # convert to radians
start <- 45 # set starting point for bars where tips end
scaledat <- .5 # scale to apply so its not disproportional to plotting area
radius <- (bardata*scaledat) + start # "radius" determines size of bars
segments(cos(angle)*start, sin(angle)*start, cos(angle)*radius, sin(angle)*radius, lwd=0.5, col="black", lend=1)
dev.off()

# 1My:

# Tree
tree <- read.tree('~/Documents/research/Tyranni/v2/tree_building/examl/final_timetrees/T400F_1My_collapsed.tre')
tree <- force.ultrametric(tree)
tree <- ladderize(tree)

# Get es values
nh.table <- read.table("../text_files/nh_mean_median_1My.txt")
nh <- nh.table$ht.median
names(nh) <- rownames(nh.table)

# Plot (based on code from Rafael Maia)
bardata <- nh[tree$tip.label]

# plot tree
pdf("1My_median.pdf", width=8, height=8)
par(mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(tree, type='fan', show.tip.label=F, edge.width=0.5, x.lim=c(-70,70), y.lim=c(-70,70), edge.color="gray")
# set angles, convert to radians
angle <- seq(0,360, length.out=Ntip(tree)+1)  # to rotate so it starts counting on right
angle <- angle[-length(angle)] # remove last one so it doesn't overlap with first one
angle <- angle*pi/180 # convert to radians
start <- 45 # set starting point for bars where tips end
scaledat <- .5 # scale to apply so its not disproportional to plotting area
radius <- (bardata*scaledat) + start # "radius" determines size of bars
segments(cos(angle)*start, sin(angle)*start, cos(angle)*radius, sin(angle)*radius, lwd=0.5, col="black", lend=1)
dev.off()

# 2My:

# Tree
tree <- read.tree('~/Documents/research/Tyranni/v2/tree_building/examl/final_timetrees/T400F_2My_collapsed.tre')
tree <- force.ultrametric(tree)
tree <- ladderize(tree)

# Get es values
nh.table <- read.table("../text_files/nh_mean_median_2My.txt")
nh <- nh.table$ht.median
names(nh) <- rownames(nh.table)

# Plot (based on code from Rafael Maia)
bardata <- nh[tree$tip.label]

# plot tree
pdf("2My_median.pdf", width=8, height=8)
par(mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(tree, type='fan', show.tip.label=F, edge.width=0.5, x.lim=c(-70,70), y.lim=c(-70,70), edge.color="gray")
# set angles, convert to radians
angle <- seq(0,360, length.out=Ntip(tree)+1)  # to rotate so it starts counting on right
angle <- angle[-length(angle)] # remove last one so it doesn't overlap with first one
angle <- angle*pi/180 # convert to radians
start <- 45 # set starting point for bars where tips end
scaledat <- .5 # scale to apply so its not disproportional to plotting area
radius <- (bardata*scaledat) + start # "radius" determines size of bars
segments(cos(angle)*start, sin(angle)*start, cos(angle)*radius, sin(angle)*radius, lwd=0.5, col="black", lend=1)
dev.off()

# dupsdropped:

# Tree
tree <- read.tree('~/Documents/research/Tyranni/v2/tree_building/examl/final_timetrees/T400F_dupsdropped.tre')
tree <- force.ultrametric(tree)
tree <- ladderize(tree)

# Get es values
nh.table <- read.table("../text_files/nh_mean_median_dupsdropped.txt")
nh <- nh.table$ht.median
names(nh) <- rownames(nh.table)

# Plot (based on code from Rafael Maia)
bardata <- nh[tree$tip.label]

# plot tree
pdf("dupsdropped_median.pdf", width=8, height=8)
par(mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(tree, type='fan', show.tip.label=F, edge.width=0.5, x.lim=c(-70,70), y.lim=c(-70,70), edge.color="gray")
# set angles, convert to radians
angle <- seq(0,360, length.out=Ntip(tree)+1)  # to rotate so it starts counting on right
angle <- angle[-length(angle)] # remove last one so it doesn't overlap with first one
angle <- angle*pi/180 # convert to radians
start <- 45 # set starting point for bars where tips end
scaledat <- .5 # scale to apply so its not disproportional to plotting area
radius <- (bardata*scaledat) + start # "radius" determines size of bars
segments(cos(angle)*start, sin(angle)*start, cos(angle)*radius, sin(angle)*radius, lwd=0.5, col="black", lend=1)
dev.off()

# HGAPF:

# Tree
tree <- read.tree('~/Documents/research/Tyranni/v2/tree_building/examl/final_timetrees/HGAPF_AOS_HowardMoore.tre')
tree <- force.ultrametric(tree)
tree <- ladderize(tree)

# Get es values
nh.table <- read.table("../text_files/nh_mean_median_HGAPF_AOSHM.txt")
nh <- nh.table$ht.median
names(nh) <- rownames(nh.table)

# Plot (based on code from Rafael Maia)
bardata <- nh[tree$tip.label]

# plot tree
pdf("HGAPF_AOSHM_median.pdf", width=8, height=8)
par(mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(tree, type='fan', show.tip.label=F, edge.width=0.5, x.lim=c(-70,70), y.lim=c(-70,70), edge.color="gray")
# set angles, convert to radians
angle <- seq(0,360, length.out=Ntip(tree)+1)  # to rotate so it starts counting on right
angle <- angle[-length(angle)] # remove last one so it doesn't overlap with first one
angle <- angle*pi/180 # convert to radians
start <- 45 # set starting point for bars where tips end
scaledat <- .5 # scale to apply so its not disproportional to plotting area
radius <- (bardata*scaledat) + start # "radius" determines size of bars
segments(cos(angle)*start, sin(angle)*start, cos(angle)*radius, sin(angle)*radius, lwd=0.5, col="black", lend=1)
dev.off()

# Astral:

# Tree
tree <- read.tree('~/Documents/research/Tyranni/v2/tree_building/examl/final_timetrees/T400F_Astral_AOS_HowardMoore.tre')
tree <- force.ultrametric(tree)
tree <- ladderize(tree)

# Get es values
nh.table <- read.table("../text_files/nh_mean_median_Astral_AOSHM.txt")
nh <- nh.table$ht.median
names(nh) <- rownames(nh.table)

# Plot (based on code from Rafael Maia)
bardata <- nh[tree$tip.label]

# plot tree
pdf("Astral_AOSHM_median.pdf", width=8, height=8)
par(mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(tree, type='fan', show.tip.label=F, edge.width=0.5, x.lim=c(-70,70), y.lim=c(-70,70), edge.color="gray")
# set angles, convert to radians
angle <- seq(0,360, length.out=Ntip(tree)+1)  # to rotate so it starts counting on right
angle <- angle[-length(angle)] # remove last one so it doesn't overlap with first one
angle <- angle*pi/180 # convert to radians
start <- 45 # set starting point for bars where tips end
scaledat <- .5 # scale to apply so its not disproportional to plotting area
radius <- (bardata*scaledat) + start # "radius" determines size of bars
segments(cos(angle)*start, sin(angle)*start, cos(angle)*radius, sin(angle)*radius, lwd=0.5, col="black", lend=1)
dev.off()

# Astral missingdropped:

# Tree
tree <- read.tree('~/Documents/research/Tyranni/v2/tree_building/examl/final_timetrees/T400F_Astral_AOS_HowardMoore_highmissingdropped.tre')
tree <- force.ultrametric(tree)
tree <- ladderize(tree)

# Get es values
nh.table <- read.table("../text_files/nh_mean_median_Astral_AOSHM_missingdropped.txt")
nh <- nh.table$ht.median
names(nh) <- rownames(nh.table)

# Plot (based on code from Rafael Maia)
bardata <- nh[tree$tip.label]

# plot tree
pdf("Astral_AOSHM_missingdropped_median.pdf", width=8, height=8)
par(mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(tree, type='fan', show.tip.label=F, edge.width=0.5, x.lim=c(-70,70), y.lim=c(-70,70), edge.color="gray")
# set angles, convert to radians
angle <- seq(0,360, length.out=Ntip(tree)+1)  # to rotate so it starts counting on right
angle <- angle[-length(angle)] # remove last one so it doesn't overlap with first one
angle <- angle*pi/180 # convert to radians
start <- 45 # set starting point for bars where tips end
scaledat <- .5 # scale to apply so its not disproportional to plotting area
radius <- (bardata*scaledat) + start # "radius" determines size of bars
segments(cos(angle)*start, sin(angle)*start, cos(angle)*radius, sin(angle)*radius, lwd=0.5, col="black", lend=1)
dev.off()


