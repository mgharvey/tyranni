...
# Measure gamma for full tree
gamma <- ltt(tree, plot=FALSE, gamma=FALSE)
print(gammatest(gamma))

results <- c("Clade", "Gamma", "P-Value")
results <- rbind(results, c("All", gammatest(gamma)$gamma, gammatest(gamma)$p))
...
for(i in 1:length(nodes)) {
	...
	if(length(subclade$tip.label) > 20) {
		...
		print(gammatest(gamma))
		results <- rbind(results, c(family, gammatest(gamma)$gamma, gammatest(gamma)$p))

	}
}

write.csv(results, "Gamma_results.csv")
